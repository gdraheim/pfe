...
tty_interactive (void)
{
  ...
  if (!isatty (STDIN_FILENO) || P4_opt.canonical)
    return;
 ...
 */
{
  ...
  for (i = 0; i < P4_NUM_KEYS; i++)
    PFE.rawkey_string[i] = tgetstr (tckeycode[i], &tctop);

  /* another chance for F10: */
  if (PFE.rawkey_string [P4_KEY_k0 - P4_KEY_k1] == NULL)
    PFE.rawkey_string [P4_KEY_k0 - P4_KEY_k1] = tgetstr ("k;", &tctop);
  ...
  for (i = 0; i < DIM (tcctlcode); i++)
    PFE.control_string[i] = tgetstr (tcctlcode[i], &tctop);
...
t_puts (int tcidx, int n)	/* issue termcap string to terminal */
{
  if (!PFE.control_string[tcidx])
    ...
    return;
  tputs (PFE.control_string[tcidx], n, t_putc);
  fflush (stdout);
}
