...
#define OUTBUF_RING (160*25) /* about 4K */
...
int logPrintf(void *pEmul, char *fmt, ...)
{
    va_list argptr;
    int ret;
...
void k12LogString (int mask, char* hail, char* p, int size)
{
    int  i, j=0;
        ...
        if (n && p->tx_qid) 
        {
            do {
                if (tQSend (p->tx_qid, 0, 0, buf, n, OUTPUT_TQSEND_TIMEOUT) 
                  == OK)
                    k12LogString (K12_LOG_DEBUG_L(15), "sent>", buf, n);  
                else
                k12LogString (K12_LOG_DEBUG_L(15), "sent> FAIL at ", buf, n);  
                n = 0;
...
#    endif
    ...
    */
    {  /* C99: dynamic-sized auto-fields, no alloca needed, e.g. gcc 2.95.2 */
        char tasknameTX[strlen(taskname)+4];
...
c_prepare_terminal (void)
{
    ...
    if (!k12p->tx_ring)
    {
        k12p->tx_ring = rngCreate (OUTBUF_RING);
        ...
        {  
            ...
            switch (msg->type) 
            {
             case K12_EMU_DATA_REQ: 
                 K12LogMsg2 ( K12_LOG_DEBUG_L(11), "got>", "K12_EMU_DATA_REQ(%i) len=%i\n",
                   msg->type, k12p->frm_datalen -_sizeof_subhead_t); 
...
c_put_flush (void)
{
    ...
    if (rngNBytes (k12p->tx_ring) > OUTBUF_RING-256) {
    ...
    if (k12p->answering                        /* command/answer facility */
      && k12p->answeridx < K12_ANSWERBUF_SIZE-1) 
    {
        register int i;
...
{ 
    ...
    {   /* memorize the string - it's gotten send on each term (re-)open */
        strncpy (k12p->f[num], name, K12_TERM_FKNAME_MAX); 
