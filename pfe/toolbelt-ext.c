 ...
 *  see => [DEFINED]
 */
extern FCode (p4_bracket_undefined);
...
P4_LISTWORDS(toolbelt) =
{
    ...
    CO ("PLACE",		p4_place),
    CO ("STRING,",      	p4_parse_comma),
    CI (",\"",          	p4_parse_comma_quote),
    CO ("THIRD",        	p4_third),
    CO ("FOURTH",       	p4_fourth),
    ...
    CO ("SKIP",			p4_skip),
    CO ("BACK",			p4_back),
    CO ("/SPLIT",		p4_div_split),
    CO ("IS-WHITE",		p4_is_white),
    CO ("TRIM",			p4_trim),
    CO ("BL-SCAN",		p4_bl_scan),
    CO ("BL-SKIP",		p4_bl_skip),
    CO ("STARTS?",		p4_starts_Q),
    CO ("ENDS?",		p4_ends_Q),
    CO ("IS-DIGIT",		p4_is_digit),
    CO ("IS-ALPHA",		p4_is_alpha),
    CO ("IS-ALNUM",		p4_is_alnum),
    OC ("#BACKSPACE-CHAR", 	'\b'),
    OC ("#CHARS/LINE",  	80),
    OC ("#EOL-CHAR", 		EOL_CHAR),
    OC ("#TAB-CHAR", 		'\t'),
    OC ("MAX-N",        	CELL_MAX),
    OC ("SIGN-BIT",     	(1 << (sizeof(p4cell)-1))),
    OC ("CELL",         	sizeof(p4cell)),
    OC ("-CELL",        	- sizeof(p4cell)),
    CO ("SPLIT-NEXT-LINE", 	p4_split_next_line),
    CO ("VIEW-NEXT-LINE", 	p4_view_next_line),
    DV ("OUT",                  out),
    CO ("NEXT-WORD",            p4_next_word),
    CO ("LEXEME",               p4_lexeme),
    CI ("H#",                   p4_h_sh),
    CO ("\\\\",                 p4_backslash_backslash),
