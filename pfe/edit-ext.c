...
#define ED (*(struct p4ed*)(PFE.p[slot]))
static int slot = 0;
...
void p4ed_init (struct p4ed* p)
{
   ...
   p->search_lined.string = ED.search_str;
   p->search_lined.max_length = sizeof (ED.search_str);
   p->search_lined.history = ED.search_history;
   p->search_lined.history_max = sizeof (ED.search_history);
   p->search_lined.complete = p4_complete_dictionary;
   p->search_lined.executes = NULL;
   
   p->replace_lined.string = ED.replace_str;
   p->replace_lined.max_length = sizeof (ED.replace_str);
   p->replace_lined.history = ED.replace_history;
   p->replace_lined.history_max = sizeof (ED.replace_history);
   p->replace_lined.complete = p4_complete_dictionary;
   p->replace_lined.executes = NULL;
}
...
scr_changed (void)
{
  ...
  return memcmp (ED.blk, ED.buf, sizeof (blck)) != 0;
}
...
truncate_file (void)
{
  ...
  for (n = BLOCK_FILE->size; n; n--)
    if (!scr_empty (n - 1))
      break;
  p4_resize_file (BLOCK_FILE, n * BPBUF);
}
...
writebuf (void)
{
  ...
  if (SCR != NOBLK && scr_changed ())
	...
	stamp_screen ();
      ...
      memcpy (ED.blk, ED.buf, sizeof (blck));
      p4_update (BLOCK_FILE);
      p4_save_buffers (BLOCK_FILE);
    }
}
...
readbuf (int n)
{
  ...
  memcpy (ED.buf, ED.blk, sizeof (blck));
  SCR = n;
}
...
insertl (int row)
{
  ...
  memset (ED.buf + i, ' ', sizeof (ED.buf[15]));
}
...
deletel (int row)
{
  ...
  for (i = row; i < 15; i++)
    COPY (ED.buf[i], ED.buf[i + 1]);
  memset (ED.buf + 15, ' ', sizeof (ED.buf[15]));
}
...
deletes (void)
{
  ...
  for (n = SCR + 1; n < BLOCK_FILE->size; n++)
    scr_copy (n - 1, n);
  memset (p4_buffer (BLOCK_FILE, BLOCK_FILE->size - 1, &n), ' ', BPBUF);
  FX (p4_update);
  readbuf (SCR);
  show_screen ();
  return 1;
...
p4_edit (int n, int r, int c)
{
  ...
  logn = getenv ("LOGNAME");
  strncpy (ED.log_name, logn ? logn : LOGNAME, sizeof (ED.log_name));
...
FCode (p4_edit_text)
{
  ...
  p4_systemf ("%s %s", P4_opt.editor, 
	      p4_pocket_expanded_filename (nm+1, *nm,
...
FCode (p4_edit_error)
{
  switch (PFE.input_err.source_id)
	...
	p4_systemf ("%s +%d %s", P4_opt.editor, (int) f->n + 1, f->name);
	break;
      }
    }
}
...
P4_LISTWORDS (p4ed) =
{
    CO ("EDIT-BLOCK",	p4_edit_block),
#ifndef NO_SYSTEM
    CO ("EDIT-TEXT",	p4_edit_text),
    CO ("EDIT-ERROR",	p4_edit_error),
#endif
};
P4_COUNTWORDS (p4ed, "EDIT - builtin forth editor");

P4_LOADLIST (p4ed) =
{
    P4_LOAD_SLOT, &slot, (void*) sizeof (struct p4ed),
