...
elsif (/^([^\d]+) (\d+\.\d[\d\.]+) (\([^\(\)]*\))?/)
{
    ...
    $package = $1; 
    $version = $2;
    $program =~ s/[a-z ]*//g if $program =~ /[A-Z][a-z]+ [A-Z][a-z]+/;
}
elsif (/^((?:GNU|Free) +)?(\S+) +(.*)/)
{
    $program = $2;
...
$include{NAME} ||= "$program \\- manual page for $package $version\n";

# Man pages traditionally have the page title in caps.
my $PROGRAM = uc $program;

# Extract usage clause(s) [if any] for SYNOPSIS.
if ($help_text =~ s/^[^\w]*Usage:( +(\S+))(.*)((?:\n(?: {6}\1| *or: +\S).*)*)//m)
{
    my @syn = $2 . $3;
...
.TH $PROGRAM "$section" "$date" "$package $version" LGPL
EOT
